...
public class AllScriptsTests
{
    ...
    [Test]
    public void PlayerStealth_NoiseRadiusUsesControllerState()
    {
        var player = CreateGO("Player");
        ...
        var stealth = player.AddComponent<PlayerStealth>();
        stealth.controller = controller;

        var crouchField = typeof(PlayerController).GetField("isCrouching", BindingFlags.NonPublic | BindingFlags.Instance);
        crouchField.SetValue(controller, true);
        Assert.AreEqual(0f, stealth.GetCurrentNoiseRadius());

        crouchField.SetValue(controller, false);
        var velocityField = typeof(PlayerController).GetField("frameVelocity", BindingFlags.NonPublic | BindingFlags.Instance);
        velocityField.SetValue(controller, new Vector3(1f, 0f, 0f));
        Assert.AreEqual(stealth.walkNoiseRadius, stealth.GetCurrentNoiseRadius());
    }
    ...
    public void SwordWeapon_BlockingStateChanges()
    {
        ...
        var startBlock = typeof(SwordWeapon).GetMethod("StartBlock", BindingFlags.Instance | BindingFlags.NonPublic);
        var stopBlock = typeof(SwordWeapon).GetMethod("StopBlock", BindingFlags.Instance | BindingFlags.NonPublic);

        startBlock.Invoke(sword, null);
        Assert.True(sword.blocking);

        stopBlock.Invoke(sword, null);
    ...
    [Test]
    public void ActorAI_CanBeStealthKilledChecksConditions()
    {
        var enemy = CreateGO("Enemy");
        enemy.AddComponent<AudioSource>();
        enemy.AddComponent<NavMeshAgent>();
        var ai = enemy.AddComponent<ActorAI>();
        ai.faction = ActorAI.Faction.Enemy;
        ...
        typeof(PlayerController).GetField("isCrouching", BindingFlags.NonPublic | BindingFlags.Instance).SetValue(controller, true);

        Assert.True(ai.CanBeStealthKilledBy(player.transform, stealth, 2f));
    }
}
